...
| --- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| 1   | Redis session token overflow | ADK Redis session accumulates conversation history indefinitely. After ~50 messages, hits OpenAI 272K token limit. Requires manual Redis key flush to unblock user. | Users get silent responses, cannot use chat |
| 2   | Context window management    | `buildConversationContext` injects last 20 messages, but ADK session also keeps full history internally. No truncation or sliding window in the runtime.            | Compounds issue #1                          |
...
| --- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| 4   | Context window truncation | Runtime must cap ADK session to fit model's context limit. Options: sliding window, summarization, or hard truncation.  | Blocks all long conversations    |
