...
func Calculate() (VersionResult, error) {
...
func CalculateForRepo(repoPath string) (VersionResult, error) {
	...
	if !semver.IsValid(lastTag) {
		return VersionResult{}, fmt.Errorf("invalid tag %s: not a valid semver", lastTag)
	}
	...
	return VersionResult{Tag: tag, LastTag: lastTag, Bump: bump}, nil
}
...
func CalculateForPrefix(prefix string) (VersionResult, error) {
	pattern := prefix + "v*"
	lastTag := latestRemoteTag("", pattern)

	if lastTag == "" {
		tag := prefix + "v0.1.0"
		return VersionResult{Tag: tag, LastTag: "", Bump: "minor"}, nil
	}
