// Package config provides helpers for resolving sensitive configuration
// values from environment variables or volume-mounted secret files, and
// holds shared default values used across the crawbl backend.
package config
...
// Default timeout and grace-period constants shared across the crawbl backend.
// Centralizing these makes tuning a single-point edit.
const (
