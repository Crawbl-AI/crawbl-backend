...
func TestChunkText(t *testing.T) {
	...
	tests := []struct {
		...
		minChunk   int
		wantCount  int
		wantJoined string // non-empty: check all chunks joined contain this
	}{
		{
		...
		},
		{
			...
			overlap:   10,
			minChunk:  5,
			wantCount: 2, // at least 2 chunks expected
		},
		{
		...
		},
		{
			...
			overlap:   10,
			minChunk:  5,
			wantCount: 1, // no sentence boundary → single oversized chunk (acceptable for auto-ingest)
		},
	}
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// For "chunks below minChunk" case we just verify no panic and no chunks > maxSize.
