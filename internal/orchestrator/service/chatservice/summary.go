...
func (s *service) GetWorkspaceSummary(ctx context.Context, opts *orchestratorservice.GetWorkspaceSummaryOpts) (*orchestrator.WorkspaceSummary, *merrors.Error) {
	if opts == nil || opts.Sess == nil || strings.TrimSpace(opts.WorkspaceID) == "" {
		return nil, merrors.ErrInvalidInput
	}

	agents, mErr := s.agentRepo.ListByWorkspaceID(ctx, opts.Sess, opts.WorkspaceID)
	if mErr != nil {
		return nil, mErr
	}
	...
	conversations, mErr := s.conversationRepo.ListByWorkspaceID(ctx, opts.Sess, opts.WorkspaceID)
	if mErr != nil || len(conversations) == 0 {
		return summary, nil
	}

	latestMsg, mErr := s.messageRepo.GetLatestByConversationID(ctx, opts.Sess, conversations[0].ID)
	if mErr != nil {
