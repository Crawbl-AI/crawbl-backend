...
// shares the MCP toolset so it can use orchestrator tools (for
// example to look up the user's workspace time zone via
// get_user_profile).
func NewEve(model adkmodel.LLM, mcpToolset adktool.Toolset, instruction, description string) (adkagent.Agent, error) {
	if model == nil {
