...
func Load(args []string, stderr io.Writer) (Config, error) {
	...
	fs.StringVar(&cfg.GRPCListen, "grpc-listen", config.StringOr("CRAWBL_GRPC_LISTEN", cfg.GRPCListen), "gRPC listen address (host:port)")
	...
	fs.StringVar(&cfg.OpenAI.ModelName, "openai-model", config.StringOr("CRAWBL_OPENAI_MODEL", cfg.OpenAI.ModelName), "OpenAI model name")
	...
	fs.StringVar(&cfg.SearXNGEndpoint, "searxng-endpoint", config.StringOr("CRAWBL_SEARXNG_ENDPOINT", cfg.SearXNGEndpoint), "SearXNG meta-search base URL used by the web_search_tool")
