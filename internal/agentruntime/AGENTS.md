...
Go implementation of the per-workspace agent runtime pod. Exposes a gRPC service (Converse bidi stream + Memory unary RPCs) that the orchestrator calls to drive multi-agent conversations using Google's ADK-Go framework with a Redis-backed session layer and Postgres-backed durable memory.

## Layout

- `proto/v1/` — Generated gRPC bindings for `AgentRuntime` (Converse) and `Memory` services. Do not edit `.pb.go` files directly; regenerate with `make generate`.
- `server/` — gRPC server wiring: registers `AgentRuntime` + `Memory` handlers, installs HMAC auth interceptor from `internal/pkg/grpc`, manages graceful shutdown lifecycle.
...
- `tools/` — Tool catalog and tool implementations. `catalog.go` holds typed name constants and wraps `migrations/orchestrator/seed`; `tools/local/` has in-process tools (`web_fetch`, `web_search_tool`, `files`); `tools/mcp/` bridges `orchestrator__*` tools to the orchestrator's MCP endpoint via HMAC-signed HTTP.
...
- `memory/` — Postgres-backed durable memory store (`agent_memories` table). Implements the `memory.Store` interface consumed by the gRPC Memory service.
...
- `config/` — Typed `Config` struct loaded once at startup from CLI flags and env vars. No globals; passed by value into every subpackage. Reuses `internal/pkg/database` and `internal/pkg/redisclient` config types.
...
- **Event list cap** — `maxSessionEvents = 60` events per session (≈15 turns). `AppendEvent` calls `LTrim` on every write; old events are silently dropped. Callers that need full history must use the Postgres memory store.
