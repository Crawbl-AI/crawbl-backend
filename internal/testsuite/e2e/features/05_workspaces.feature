Feature: Workspace management
  ...
  Background:
    Given a new test user "dave"
    And user "dave" has signed up

  Scenario: Default workspace is created on sign-up
    When user "dave" sends a GET request to "/v1/workspaces"
  ...
  Scenario: Get single workspace by ID
    Given user "dave" has a workspace saved as "workspace_id"
    When user "dave" sends a GET request to "/v1/workspaces/{workspace_id}"
    Then the response status should be 200
  ...
  Scenario: Workspace has default agents
    Given user "dave" has a workspace saved as "workspace_id"
    When user "dave" sends a GET request to "/v1/workspaces/{workspace_id}/agents"
    Then the response status should be 200
