// Package webhook builds Kubernetes manifests for UserSwarm reconciliation.
//
// This package contains helpers that are not tied to any specific CRD or domain:
//
//	kube_names.go     — Resource name truncation with hash suffixes, deterministic checksums
//	kube_builders.go  — Common K8s struct builders (TypeMeta, SecurityContext, EnvFrom, etc.)
//	kube_types.go     — Type constants
package webhook
